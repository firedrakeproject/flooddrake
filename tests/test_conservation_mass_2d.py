...
# test conservation of mass on flat 0 bed with both flat source and un-flat source
...
def test_conservation_mass_2d_flat_source():
    ...
    g=Function(V)
    g.interpolate(Expression(['pow(x[0]-0.5,2) + pow(x[1]-0.5,2)< 0.05 ? 0.8 : (pow(x[0]-0.5,2) + pow(x[1]-0.5,2)< 0.05 ? -1.0 : 0.8)',0,0]))
    
    # setup bed
    bed=Function(V)
    bed.interpolate(Expression(["0",0,0])) # pointless trivial second dimension
    ...
    if mass_diff>1e-5:
        raise AssertionError('failed conservation of mass with flat source')



if __name__ == "__main__":
    test_conservation_mass_2d_flat_source()
...
if __name__ == "__main__":
    test_conservation_mass_2d_unflat_source()
