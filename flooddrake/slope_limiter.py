...
def SlopeLimiter(w, b, VCG):
    ...
    par_loop(
        vert_max_min_kernel, dx, {
            "cell_av": (
                c, READ), "vert_cell_min": (
                    v_min_cg, RW), "vert_cell_max": (
                        v_max_cg, RW)})

    par_loop(
        slope_limiter_kernel, dx, {
            "vert_cell_dg": (
                v_func, RW), "vert_cell_min": (
                v_min_cg, READ), "vert_cell_max": (
                    v_max_cg, READ), "cell_av": (
                        c, READ), "cell_depth": (
                            h, READ)})
